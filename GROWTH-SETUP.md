...
- 공유 형식: `?s=1LIsdPZ`. 첫 글자는 형식 버전 `1`이며, 이후 번호당 한 글자씩 6글자가 1게임입니다. 1게임 코드는 7자, 5게임 코드는 31자입니다. 별도 서버나 만료 기간 없이 번호와 추첨 순서를 보존합니다. 표시용 영수증만 오름차순으로 정렬합니다.
- 기존 `?numbers=12,9,45,30,16,26;30,20,15,8,3,1` 형식도 읽을 수 있습니다. 중복되거나 두 형식이 혼합된 파라미터는 거부합니다. 이 코드는 암호화나 구매 인증이 아닌 번호의 짧은 표현입니다.
...
- 영수증의 각 게임(A~E) 번호 옆 재생 버튼으로 바로 3D 추첨을 재현합니다. 추첨기 화면으로 이동하고 게임 이름·재생 상태·영수증 복귀 링크를 표시합니다. 재생 중에는 중복 실행을 막고 완료 시 버튼을 복구합니다. PNG에는 재생 버튼을 넣지 않습니다.
...
페이지뷰는 GA4 기본 수집을 사용하며 전달 URL에서 `numbers`, `s`, hash를 제외합니다. 명시된 UTM은 유지합니다. 자체 이벤트에는 번호를 넣지 않습니다. GA4 향상된 측정의 브라우저 기록 기반 페이지뷰/외부 링크 자동 측정 등 별도 수집 설정은 관리자에서 확인하세요.
...
공유 링크에는 UTM을 붙이지 않습니다. 공유 파라미터가 있고 별도 `utm_source`가 없으면 분석에 전달하는 URL에만 `utm_source=lotto_share&utm_medium=referral`을 추가합니다. 브라우저 주소와 복사한 링크는 짧게 유지됩니다. 배포 후 GA4 실시간 보고서에서 페이지뷰 → 추첨 시작 → 완료 → 공유를 확인합니다. 실제 ID/계정이 없으면 보고서 수집 검증은 불가능합니다.
...
영수증 광고는 `ticket-ads.css` / `ticket-ads.js`에서 관리합니다. 1200px 이상은 좌우 300×250 두 자리, 그 미만은 공유·저장 버튼 아래 한 자리입니다. 기존 광고 단위 `5213473237`을 재사용하며 기존 단독 광고 영역은 제거했습니다. 실제 화면에 보일 때 한 번만 요청하므로 반복 추첨으로 광고를 새로 요청하지 않습니다. 광고 주변에서는 고정 추첨 버튼을 잠시 숨겨 겹침을 방지합니다. 저장하는 PNG에는 광고가 포함되지 않습니다. 실제 광고 게재 여부는 AdSense 응답에 달려 있습니다.
